...
-- ============================================================
-- RANKING DIARIO COMPARTILHADO (antes era so localStorage, ou
-- seja, cada aparelho tinha o proprio ranking isolado - agora
-- todos os jogadores aparecem no mesmo ranking de verdade)
-- ============================================================

create table if not exists public.daily_rank (
  user_id uuid references auth.users(id) on delete cascade,
  mode text not null,                -- 'survival' ou 'pvp'
  cycle_date date not null,          -- ciclo que fecha as 21h daquele dia
  username text not null,
  points integer not null default 0,
  matches integer not null default 0,
  best_placement integer not null default 5,
  updated_at timestamptz not null default now(),
  primary key (user_id, mode, cycle_date)
);
alter table public.daily_rank enable row level security;

-- ranking e publico pra LER (e o objetivo: ver todos os jogadores)
create policy "Qualquer um pode ler o ranking diario"
  on public.daily_rank for select
  using (true);

-- mas cada jogador so escreve na PROPRIA linha
create policy "Jogador insere so a propria pontuacao diaria"
  on public.daily_rank for insert
  with check (auth.uid() = user_id);
create policy "Jogador atualiza so a propria pontuacao diaria"
  on public.daily_rank for update
  using (auth.uid() = user_id);

-- Soma pontos de forma ATOMICA no servidor. Importante: fazer a soma aqui
-- (e nao ler-somar-escrever no navegador) evita perder pontos se o jogador
-- terminar duas partidas quase ao mesmo tempo em aparelhos diferentes.
create or replace function public.add_daily_rank_points(
  p_mode text, p_cycle_date date, p_username text, p_points integer, p_placement integer
) returns void
language plpgsql
security definer
set search_path = public
as $$
begin
  insert into public.daily_rank(user_id, mode, cycle_date, username, points, matches, best_placement)
    values (auth.uid(), p_mode, p_cycle_date, p_username, p_points, 1, p_placement)
...
grant execute on function public.add_daily_rank_points(text, date, text, integer, integer) to authenticated;
